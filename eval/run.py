...
def main():
    ...
    for _, model_name in enumerate(args.model):
        ...
        _pred_root_env = os.environ.get("pred_root", None)
        if _pred_root_env is not None:
            pred_root = _pred_root_env
            # Flat layout: keep meta in the same dir, no per-model subfolder.
            pred_root_meta = pred_root
        else:
            pred_root_meta = osp.join(args.work_dir, model_name)
        os.makedirs(pred_root_meta, exist_ok=True)

        _prev_dir = osp.join(args.work_dir, model_name)
        prev_pred_roots = ls(_prev_dir, mode='dir') if osp.isdir(_prev_dir) else []
        ...
        for _, dataset_name in enumerate(args.data):
            ...
            try:
                ...
                if RANK == 0:
                    ...
                    # Skip when pred_root and pred_root_meta resolve to the same
                    # directory (flat layout requested via env pred_root).
                    if osp.realpath(pred_root) != osp.realpath(pred_root_meta):
